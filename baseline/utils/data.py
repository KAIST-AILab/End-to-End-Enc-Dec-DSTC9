...
def write_selection_preds(dataset_walker, output_file, data_infos, sorted_pred_ids, topk=5):
    ...
    labels = [item['label'] for item in dataset_walker]
...
def pad_nested_ids(arrayss, padding, max_length=-1):
    if max_length < 0:
        max_length = max(len(array) for arrays in arrayss for array in arrays)
